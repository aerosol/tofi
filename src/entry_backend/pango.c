...
static void rounded_rectangle(cairo_t *cr, uint32_t width, uint32_t height,
                              uint32_t r) {
  cairo_new_path(cr);
...
static void render_text_themed(cairo_t *cr, struct entry *entry,
                               const char *text, const struct text_theme *theme,
  ...
  rounded_rectangle(cr, ceil(ink_rect->width + padding_left + padding_right),
...
static void render_input(cairo_t *cr, PangoLayout *layout, const char *text,
                         uint32_t text_length, const struct text_theme *theme,
  ...
  if (cursor_position == text_length && cursor_theme->show) {
    ...
    extra_cursor_advance +=
        logical_rect->width - logical_rect->x - ink_rect->width;
  }
  ...
  if (theme->background_color.a != 0) {
    ...
    rounded_rectangle(cr,
                      ceil(ink_rect->width + extra_cursor_advance +
  ...
  switch (cursor_theme->style) {
  case CURSOR_STYLE_BAR:
    rounded_rectangle(cr, cursor_theme->thickness, logical_rect->height,
                      cursor_theme->corner_radius);
    cairo_fill(cr);
    break;
  case CURSOR_STYLE_BLOCK:
    rounded_rectangle(cr, cursor_width, logical_rect->height,
                      cursor_theme->corner_radius);
  ...
  case CURSOR_STYLE_UNDERSCORE:
    ...
    rounded_rectangle(cr, cursor_width, cursor_theme->thickness,
                      cursor_theme->corner_radius);
    cairo_fill(cr);
    break;
  }
...
void entry_backend_pango_init(struct entry *entry, uint32_t *width,
                              uint32_t *height) {
  ...
  if (entry->font_variations[0] != 0) {
    pango_font_description_set_variations(font_description,
  ...
  if (hb_font_get_glyph_from_name(hb_font, "m", -1, &m_codepoint)) {
    entry->cursor_theme.em_width =
        (double)hb_font_get_glyph_h_advance(hb_font, m_codepoint) / PANGO_SCALE;
  } else {
    entry->cursor_theme.em_width =
  ...
  entry->cursor_theme.underline_depth =
      (double)(pango_font_metrics_get_ascent(metrics) -
               pango_font_metrics_get_underline_position(metrics)) /
      PANGO_SCALE;
  if (entry->cursor_theme.style == CURSOR_STYLE_UNDERSCORE &&
      !entry->cursor_theme.thickness_specified) {
    entry->cursor_theme.thickness =
        pango_font_metrics_get_underline_thickness(metrics) / PANGO_SCALE;
  }
...
static bool size_overflows(struct entry *entry, uint32_t width,
                           uint32_t height) {
...
void entry_backend_pango_update(struct entry *entry) {
  ...
  PangoRectangle ink_rect;
  PangoRectangle logical_rect;
  render_text_themed(cr, entry, entry->prompt_text, &entry->prompt_theme,
  ...
  if (entry->input_utf8_length == 0) {
    render_input(cr, layout, entry->placeholder_text,
                 utf8_strlen(entry->placeholder_text),
                 &entry->placeholder_theme, 0, &entry->cursor_theme, &ink_rect,
                 &logical_rect);
    ...
    render_input(cr, layout, buf, entry->input_utf32_length,
                 &entry->input_theme, entry->cursor_position,
                 &entry->cursor_theme, &ink_rect, &logical_rect);
    free(buf);
  } else {
    render_input(cr, layout, entry->input_utf8, entry->input_utf32_length,
                 &entry->input_theme, entry->cursor_position,
                 &entry->cursor_theme, &ink_rect, &logical_rect);
  }
  ...
  for (i = 0; i < num_results; i++) {
    if (entry->horizontal) {
      cairo_translate(
          cr, logical_rect.x + logical_rect.width + entry->result_spacing, 0);
    } else {
    ...
    if (i != entry->selection || (entry->selection_highlight_color.a == 0)) {
      ...
      for (int pass = 0; pass < 2; pass++) {
        ...
        pango_layout_set_markup(layout, str, prematch_len);
        pango_cairo_update_layout(cr, layout);
        pango_cairo_show_layout(cr, layout);
        pango_layout_get_pixel_extents(entry->pango.layout, &ink_subrect,
                                       &logical_subrect);
        ...
        if (prematch_len != -1) {
          ...
          cairo_set_source_rgba(cr, color.r, color.g, color.b, color.a);
          pango_layout_set_markup(layout, &str[prematch_len], match_len);
          pango_cairo_update_layout(cr, layout);
          pango_cairo_show_layout(cr, layout);
          pango_layout_get_pixel_extents(entry->pango.layout, &ink_subrect,
                                         &logical_subrect);
          if (prematch_len == 0) {
            ...
            ink_rect.width = logical_rect.width - ink_rect.x + ink_subrect.x +
                             ink_subrect.width;
        ...
        if (postmatch_len != -1) {
          ...
          color = entry->selection_theme.foreground_color;
          cairo_set_source_rgba(cr, color.r, color.g, color.b, color.a);
          pango_layout_set_markup(layout, &str[prematch_len + match_len], -1);
          pango_cairo_update_layout(cr, layout);
          pango_cairo_show_layout(cr, layout);
          pango_layout_get_pixel_extents(entry->pango.layout, &ink_subrect,
                                         &logical_subrect);
        ...
        if (entry->selection_theme.background_color.a == 0) {
          ...
          cairo_translate(cr, floor(-padding.left + ink_rect.x), -padding.top);
          rounded_rectangle(
              cr, ceil(ink_rect.width + padding.left + padding.right),
              ceil(logical_rect.height + padding.top + padding.bottom),
